...
main()
{
	...
	// --- Real hardware captures (via tools/uvc_descriptor_dump on 1bcf:0001) ---
	// The AUKEY PC-LM1E: MJPEG + YUY2, 6 resolutions, up to 1080p.
	{
		// VS_FRAME_MJPEG: 1280x720, 7 discrete intervals, bLength 54. (MJPEG
		// frame descriptors share the uncompressed frame layout.)
		static const uint8 mjpegFrame[] = {
			0x36,0x24,0x07,0x01,0x00,0x00,0x05,0xd0,0x02,0x00,0x00,0x65,
			0x04,0x00,0x00,0x5e,0x1a,0x00,0x20,0x1c,0x00,0x15,0x16,0x05,
			0x00,0x07,0x15,0x16,0x05,0x00,0x80,0x1a,0x06,0x00,0x20,0xa1,
			0x07,0x00,0x2a,0x2c,0x0a,0x00,0x40,0x42,0x0f,0x00,0x55,0x58,
			0x14,0x00,0x80,0x84,0x1e,0x00
		};
		UVCFrameDescCheck mf = UVCCheckFrameDescriptor(mjpegFrame,
			sizeof(mjpegFrame));
		Expect("real: AUKEY 1280x720 MJPEG frame", mf.valid && mf.width == 1280
			&& mf.height == 720 && mf.maxVideoFrameSize == 1280 * 720 * 2
			&& mf.frameIntervalType == 7);

		// VS_FORMAT_UNCOMPRESSED: formatIndex 1, 6 frames, YUY2 GUID, 16 bpp.
		static const uint8 uncFormat[] = {
			0x1b,0x24,0x04,0x01,0x06, 0x59,0x55,0x59,0x32,0x00,0x00,0x10,0x00,
			0x80,0x00,0x00,0xaa,0x00,0x38,0x9b,0x71, 0x10,0x01,0x00,0x00,0x00,0x00
		};
		UVCUncompressedFormatCheck uf = UVCCheckUncompressedFormatDescriptor(
			uncFormat, sizeof(uncFormat));
		Expect("real: AUKEY YUY2 format", uf.valid && uf.formatIndex == 1
			&& uf.numFrameDescriptors == 6 && uf.bitsPerPixel == 0x10
			&& uf.guid[0] == 0x59 && uf.guid[3] == 0x32);
	}
